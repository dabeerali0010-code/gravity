...
(function() {
    ...
    function collectImages() {
        currentImages = [];
        // City card images
        document.querySelectorAll('.city-img-wrap').forEach(wrap => {
            const img = wrap.querySelector('img');
            const title = wrap.closest('.city-card').querySelector('h3').textContent;
            currentImages.push({ src: img.src, alt: title });
        });
        // Location card images
        document.querySelectorAll('.loc-img').forEach(wrap => {
            const img = wrap.querySelector('img');
            const title = wrap.querySelector('.loc-title') ? wrap.querySelector('.loc-title').textContent : img.alt;
            currentImages.push({ src: img.src, alt: title });
        });
    }
    ...
    // Click handlers for city images
    document.addEventListener('click', function(e) {
        const cityWrap = e.target.closest('.city-img-wrap');
        if (cityWrap) {
            collectImages();
            const allCityWraps = Array.from(document.querySelectorAll('.city-img-wrap'));
            const idx = allCityWraps.indexOf(cityWrap);
            openLightbox(idx);
            return;
        }

        const locWrap = e.target.closest('.loc-img');
        if (locWrap) {
            collectImages();
            const allLocWraps = Array.from(document.querySelectorAll('.loc-img'));
            const cityCount = document.querySelectorAll('.city-img-wrap').length;
            const idx = allLocWraps.indexOf(locWrap) + cityCount;
            openLightbox(idx);
        }
    });
